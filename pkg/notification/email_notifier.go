...
// EmailNotifier implements the Sender interface for email
...
// Send implements the Sender interface for email
func (e *EmailNotifier) Send(message Message) error {
	...
	subject := fmt.Sprintf("Site Status Alert: %s is %s", message.Event.SiteURL, message.Event.Status)
	body := fmt.Sprintf(`
Site Status Alert
...
Message: %s
`, message.Event.SiteURL, message.Event.Status, message.Event.OccurredAt.String(), message.Event.Message)
